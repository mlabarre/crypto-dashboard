...
let fill = (data) => {
    $.each(data, (line) => {
        ...
        row += `<td><i onclick="deleteTransaction('${data[line].id}')" class="fa-regular fa-trash-can red" title="${titleFill1}"></i></td>`
        row += `<td><i onclick="editTransaction('${data[line].id}')" class="fas fa-edit green" title="${titleFill2}"></i></td>`
        row += '</tr>';
