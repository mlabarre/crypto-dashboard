...
let interpretFlowFr = (t, wallet) => {
    ...
    if (t.type === "purchase") {
        return {id: t._id, msg: `${d} : Achat de ${t.tokens} ${t.symbol} quoté à ${fd(t.quotation)} ${fiat} ${sup} avec ${fd(t.feeInFiat)} ${fiat} de ` +
        `frais. ${comment}`};
    } else if (t.type === "sale") {
         return {id: t._id, msg: `${d} : Vente de ${t.tokens} ${t.symbol} quoté à ${fd(t.quotation)} ${fiat} ${sup}avec ${fd(t.feeInFiat)} ${fiat} de ` +
        `frais. ${comment}`};
    } else if (t.type === "swap") {
        return {id: t._id, msg: `${d} : Echange ${sup}de ${t.outputTokens} ${t.outputSymbol} coté à ${fd(t.outputTokenQuotation)} ` +
        `${t.outputTokenQuotationCurrency} en ${t.inputTokens} ${t.inputSymbol} coté à ${fd(t.inputTokenQuotation)} ` +
        ...
        `(reçu ${t.receiveTokens}) avec ${fd(t.fee)} ${t.feeCurrency} de frais. ${comment}`};
        ...
        return {id: t._id, msg: `${d} : Envoi de ${t.sendTokens} ${t.symbol} à partir du wallet ${t.sendWallet} sur le wallet ` +
            `${t.receiveWallet} (reçu ${t.receiveTokens}) avec ${fd(t.fee)} ${t.feeCurrency} de frais. ${comment}`};
    }
}
...
let interpretFlowEn = (t, wallet) => {
    ...
    let fiat = config.get('fiat_currency').toUpperCase();
    if (t.type === "purchase") {
        return {id: t._id, msg: `${d} : Purchase ${t.tokens} ${t.symbol} quoted at ${fd(t.quotation)} ${fiat} ${sup} with fee of ${fd(t.feeInFiat)} ${fiat}` +
            `${comment}`};
    } else if (t.type === "sale") {
        return {id: t._id, msg: `${d} : Sale ${t.tokens} ${t.symbol} quoted at ${fd(t.quotation)} ${fiat} ${sup}with fee of ${fd(t.feeInFiat)} ${fiat}` +
            `${comment}`};
    } else if (t.type === "swap") {
        return {id: t._id, msg: `${d} : Swap ${sup}of ${t.outputTokens} ${t.outputSymbol} quoted at ${fd(t.outputTokenQuotation)} ` +
            `${t.outputTokenQuotationCurrency} in ${t.inputTokens} ${t.inputSymbol} quoted at ${fd(t.inputTokenQuotation)} ` +
        ...
        return {id: t._id, msg: `${d} : Send ${t.sendTokens} ${t.symbol} ${sup}to the wallet ${t.receiveWallet} ` +
            `(received ${t.receiveTokens}) with fee of ${fd(t.fee)} ${t.feeCurrency}. ${comment}`};
    } else if (t.type === "send" && t.receiveWallet === wallet) {
        if (wallet === "") {
        ...
        return {id: t._id, msg: `${d} : Send ${t.sendTokens} ${t.symbol} from wallet ${t.sendWallet} to the wallet ` +
            `${t.receiveWallet} (received ${t.receiveTokens}) with fee of ${fd(t.fee)} ${t.feeCurrency}. ${comment}`};
    }
}
